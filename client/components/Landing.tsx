...
const featureKeys: { titleKey: 'featureSpatialAudioTitle' | 'featureScreenSharingTitle' | 'featureSharedCanvasTitle' | 'featureCollaborationTitle' | 'featureSpatialMediaPlayerTitle'; descKey: 'featureSpatialAudioDesc' | 'featureScreenSharingDesc' | 'featureSharedCanvasDesc' | 'featureCollaborationDesc' | 'featureSpatialMediaPlayerDesc'; icon: JSX.Element }[] = [
  ...
  {
    titleKey: 'featureScreenSharingTitle',
    descKey: 'featureScreenSharingDesc',
    icon: (
      <svg class="shrink-0" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
  ...
  {
    titleKey: 'featureSharedCanvasTitle',
    descKey: 'featureSharedCanvasDesc',
    icon: (
      <svg class="shrink-0" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
  ...
  },
  {
    ...
    descKey: 'featureCollaborationDesc',
    icon: (
      <svg class="shrink-0" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M12 20h9" />
    ...
    <div id="landing-page" class="fixed inset-0 flex items-center justify-center bg-[linear-gradient(135deg,#f0f0ff_0%,#e8e0f0_40%,#ddeeff_100%)] z-[1000] overflow-y-auto p-8">
      ...
      <div class="max-w-[560px] w-full text-center animate-fade-in relative z-[1]">
        ...
        <div class="mb-10">
          ...
          <p class="text-xl font-semibold text-[#1e1b4b] mb-2">{t('tagline')}</p>
        ...
        <div class="flex flex-col gap-3.5 mb-10">
          {featureKeys.map((f) => (
            <div class="flex items-start gap-4 p-5 bg-white/65 backdrop-blur-[12px] border border-white/80 rounded-2xl shadow-[0_2px_12px_rgba(99,102,241,0.06)] text-left transition-all duration-(--transition-fast) hover:-translate-y-0.5 hover:shadow-[0_6px_24px_rgba(99,102,241,0.12)]">
              ...
              <div>
                <div class="text-base font-bold text-[#1e1b4b] mb-1">{t(f.titleKey)}</div>
                <div class="text-sm text-[#6b7280] leading-normal">{t(f.descKey)}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div class="mb-6">
          <form id="landing-space-form" class="flex gap-3 max-w-[480px] mx-auto max-[480px]:flex-col" onSubmit={handleSubmit}>
            ...
            />
            <button type="submit" data-testid="enter-space-btn" class="btn-primary inline-flex items-center justify-center gap-2 py-3.5 px-6 font-[inherit] text-base font-semibold border-none rounded-xl cursor-pointer bg-[linear-gradient(135deg,#6366f1_0%,#8b5cf6_50%,#a855f7_100%)] text-white whitespace-nowrap shadow-[0_4px_14px_rgba(99,102,241,0.3)] transition-all duration-(--transition-fast) hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(99,102,241,0.4)] active:translate-y-0">
              <span>{t('enterSpace')}</span>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M5 12h14M12 5l7 7-7 7" />
              </svg>
            </button>
          </form>
          <p class="mt-2 text-xs text-[#9ca3af]">{t('leaveEmptyForDemo')}</p>
        </div>

        {/* Links */}
        <div class="mb-4">
          <a href="https://github.com/srid/openspatial" target="_blank" data-testid="github-link" class="inline-flex items-center gap-2 text-[#6b7280] text-sm no-underline transition-colors duration-(--transition-fast) hover:text-[#1e1b4b]">
        ...
        {/* Footer */}
        <p class="text-sm text-[#9ca3af]">{t('footerTagline')}</p>
      </div>
    </div>
  );
};
