...
export function attachSignaling(io) {
    ...
    function getSpace(spaceId) {
        if (!spaces.has(spaceId)) {
            spaces.set(spaceId, { 
                peers: new Map(),
                screenShares: new Map() // shareId -> { peerId, username, x, y }
            });
        }
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('screen-share-position-update', ({ shareId, x, y }) => {
            if (!currentSpace) return;
            const space = spaces.get(currentSpace);
            // Update stored position
            if (space?.screenShares.has(shareId)) {
        ...
        socket.on('screen-share-started', ({ peerId: pid, shareId, x, y }) => {
            if (!currentSpace) return;
            const space = spaces.get(currentSpace);
            if (space?.peers.has(pid)) {
                space.peers.get(pid).isScreenSharing = true;
                // Store screen share with position
                space.screenShares.set(shareId, { 
                    peerId: pid, 
        ...
        socket.on('screen-share-stopped', ({ peerId: pid, shareId }) => {
            if (!currentSpace) return;
            const space = spaces.get(currentSpace);
            if (space?.peers.has(pid)) {
                space.peers.get(pid).isScreenSharing = false;
                space.screenShares.delete(shareId);
                socket.to(currentSpace).emit('screen-share-stopped', { peerId: pid, shareId });
            }
        });
